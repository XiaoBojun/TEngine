...
namespace TEngine
{
    ...
    public class PrimeTweenHelperImp : Utility.Tween.ITweenHelper
    {
        ...
        public long Custom(Vector3 startValue, Vector3 endValue, float duration, Action<Vector3> onValueChange, Ease ease = Ease.Default, int cycles = 1, CycleMode cycleMode = CycleMode.Restart,
            float startDelay = 0, float endDelay = 0, bool useUnscaledTime = false)
        {
            Tween tween = Tween.Custom(startValue, endValue, duration, onValueChange, GetEase(ease), cycles, GetCycleMode(cycleMode), startDelay, endDelay,
                useUnscaledTime);
            CacheTween(tween);
            return tween.Id;
        }
