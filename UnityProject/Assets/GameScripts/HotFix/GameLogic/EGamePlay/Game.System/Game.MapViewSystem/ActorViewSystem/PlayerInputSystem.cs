        ...
        public static void Update(Game game, PlayerInputComponent component)
        {
            ...
            if (Input.GetMouseButtonDown((int)MouseButton.RightMouse))
            {
                if (RaycastHelper.CastMapPoint(out var point))
                {
                    ...
                    StopMove(game);
                    Utility.Tween.Custom(TransformSystem.GetPosition(actor), point, time,  x => TransformSystem.ChangePosition(actor, x),Ease.Linear)
                        .OnComplete(() => { AnimationSystem.PlayFade(actor, animComp.IdleAnimation); });
